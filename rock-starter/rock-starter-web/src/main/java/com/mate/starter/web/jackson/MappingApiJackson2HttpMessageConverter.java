...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	public MappingApiJackson2HttpMessageConverter(ObjectMapper objectMapper, MateJacksonProperties properties) {
		super(objectMapper, initWriteObjectMapper(objectMapper, properties), initMediaType(properties));
	}

	private static List<MediaType> initMediaType(MateJacksonProperties properties) {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper, MateJacksonProperties properties) {
		// 拷贝 readObjectMapper
		ObjectMapper writeObjectMapper = readObjectMapper.copy();
		// long 转 字符串
		if (Boolean.TRUE.equals(properties.getBigNumToString())) {
			writeObjectMapper.registerModules(BigNumberModule.INSTANCE);
		}
		// null 处理
		if (Boolean.TRUE.equals(properties.getNullToEmpty())) {
			writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new MateBeanSerializerModifier()));
			writeObjectMapper.getSerializerProvider().setNullValueSerializer(MateBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
		}
